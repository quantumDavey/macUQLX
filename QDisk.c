...
static uw8 format_table_dd[36]={0,3,6,0x80,0x83,0x86,1,4,7,0x81,0x84,0x87,2,5,8,0x82,0x85,0x88,0,6,12,1,7,13,2,8,14,3,9,15,4,10,16,5,11,17};
static uw8 format_table_hd[36]={0,2,4,6,8,10,12,14,16,0x80,0x82,0x84,0x86,0x88,0x8a,0x8c,0x8e,0x90,1,3,5,7,9,11,13,15,17,0x81,0x83,0x85,0x87,0x89,0x8b,0x8d,0x8f,0x91};
...
void FlushSectors()
{
	...
	for (k=0; k<curr_flpfcb->bufcount; k++)
    {
		if(!curr_flpfcb->si[k].free && curr_flpfcb->si[k].changed) 
		{
			...
			if(V4) printf("flushing buffer %d\n",k);
		}
    }
}
...
int root_flen()
{
	if (curr_flpfcb->DiskType==qlwa)
		...
		return  (QDH_DIREOFBL(curr_flpfcb->qdh)<<9)+(QDH_DIREOFBY(curr_flpfcb->qdh));
}
...
default : return QERR_NI;
		...
		else
		{
			...
			curr_flpfcb->fatSectors=(3*QDH_TOTAL(buffer)/QDH_SPB(buffer)+96)/512;
			if(V4) printf("guessing %d sectors FAT length\n",curr_flpfcb->fatSectors);
		}
		
		curr_flpfcb->bufcount=curr_flpfcb->fatSectors+9; /* Fit FAT + some buffer */
		
		/* free old buffers */
		if (curr_flpfcb->buffer)
			free(curr_flpfcb->buffer);
		
		curr_flpfcb->buffer=(char *)malloc(512*(curr_flpfcb->bufcount));
		curr_flpfcb->si=(struct sectorInfo *)calloc(curr_flpfcb->bufcount,sizeof(struct sectorInfo));
...
OSErr QFOpenDisk(struct mdvFile *f)
{
	...
	if (!curr_flpfcb)
    {
		...
		if(V4) printf("calling OpenDisk, res %d\n",fd);
		...
		if(V4) printf("flp%d_ allocated as %s\n",k,qdevs[fs].mountPoints[drnum]);
...
static OSErr DiskRead(long fref,Ptr dest,long count,long offset)
{
	...
	if(V4) printf("positioning at %ld\n",offset);
	...
	if(V4) printf("calling DiskRead, res %d\n",res);
...
{	
	...
	if(V4) printf("calling DiksWrite\n");
	if (res<0) return -1;
	return 0;
}
...
static OSErr LoadLogSector(int sector,Ptr p)
{
    ...
    {
		side=QDH_LTP(curr_flpfcb->qdh)[sector%QDH_SPC(curr_flpfcb->qdh)];  /* ??????????? */
		track=sector/QDH_SPC(curr_flpfcb->qdh);
		sector=side&0x7f;
		if((side&=0x80)!=0) side=1;
		return LoadPhySector(side,track,(track*QDH_OFFSET(curr_flpfcb->qdh)+sector)%QDH_SPT(curr_flpfcb->qdh),p);
    }
}
...
static OSErr WritePhySector(uw8 side,uw16 track,uw16 sector,Ptr p)
{
	...
	if(curr_flpfcb->buffer!=nil)
    {
		abs=(long)track*QDH_SPC(curr_flpfcb->qdh)+sector+side*QDH_SPT(curr_flpfcb->qdh);
		QDH_SET_UPDTS(curr_flpfcb->qdh,QDH_UPDTS(curr_flpfcb->qdh)+1);
...
static OSErr WriteLogSector(int sector,Ptr p)
{
	...
	else
    {
		...
		sector=side&0x7f;
		if((side&=0x80)!=0) side=1;
		return WritePhySector(side,track,(track*QDH_OFFSET(curr_flpfcb->qdh)+sector)%QDH_SPT(curr_flpfcb->qdh),p);
    }
}
...
static Ptr GetSector(int sector,FileNum fileNum)
{
	...
	if(V4) printf("GetSector sector %d, file %d\n",sector,fileNum.file);
	...
	for(i=0;i<curr_flpfcb->bufcount;i++)
    {
		...
		{	
			...
			gError=0;
			if(V4) printf("... returns cached sector %d\n",i);
	...
	if(V4) printf("... uses buffer %d,\t",k);
	
	p=curr_flpfcb->buffer+((long)k<<9);
	if(!curr_flpfcb->si[k].free && curr_flpfcb->si[k].changed) WriteLogSector(curr_flpfcb->si[k].logSector,p);
	...
	if(curr_flpfcb->buffer!=nil)
		for(i=0;i<curr_flpfcb->bufcount;i++)
			...
			{	
				...
				if(!curr_flpfcb->si[i].locked) curr_flpfcb->si[i].free=true;
			}
}
...
static int QLWA_KillFileTail(FileNum fileNum, int sector)
{
	...
	if (g==group-1)
    {
		...
		curr_flpfcb->si[((char*)p1-curr_flpfcb->buffer)>>9].changed=true;
...
static int QLWA_GetFileSectNum(FileNum fileNum, int sector)
{
    ...
    {
		if(V4) printf("p-(buffer+0x40)=%d\t",(int)p-((int) (curr_flpfcb->buffer+0x40)));
		if(V4) printf("next block nr. %d\t",(uw16)RW(p));
		p=(uw16*)(curr_flpfcb->buffer+0x40)+(uw16)RW(p);
    }
	if (g==group)
...
static int FileBlockSector(FileNum fileNum, int sector) 
{
	...
	value=((w32)(fileNum.file&0x0fff)<<12)+((sector/QDH_SPB(curr_flpfcb->qdh))&0x0fff);
	w=-1;
	for(i=0;i<n;i++)
    {
		if((RL((w32*)p)>>8)==value)
	...
	if(w>=0)
    {
		w=w*QDH_SPB(curr_flpfcb->qdh)+(sector%QDH_SPB(curr_flpfcb->qdh));
    }
	else gError=ERR_NO_FILE_BLOCK;
	return w;
}
...
static Ptr QLWA_GetFreeBlock(FileNum fileNum)
{
	int g,i;
	int nb;
	uw16 *p;
...
static Ptr GetFreeBlock(FileNum fileNum, w16 block)	
{
	...
	if (curr_flpfcb->DiskType==qlwa)
		return QLWA_GetFreeBlock(fileNum);
	...
	if(QDH_FREE(curr_flpfcb->qdh)>=1)
    {
		...
		{	
			if(*p==0xfd)
			{
				...
				p[2]=(uw8)block;
				k=i<<1;
				curr_flpfcb->si[((i*6)+96)>>9].changed=true;
				curr_flpfcb->si[((i*6)+98)>>9].changed=true;
				goto found;
			}
			if(p[3]==0xfd)
			{
				p[3]=(uw8)(fileNum.file>>4);
				WW(((w16*)(p+4)), (fileNum.file<<12)|(block&0x0fff));
				k=(i<<1)+1;
...
OSErr KillFileTail(FileNum fileNum, int nBlock)	
{
    ...
    {	
		if((RW((w16*)p)>>4)==fileNum.file)
		{
			if( (w16)p[2] | ((((w16)p[1]&15)<<8) >= nBlock) ) // fixit: () around >= test
			{	
				*p=0xfd;
				QDH_SET_FREE(curr_flpfcb->qdh,QDH_FREE(curr_flpfcb->qdh)+QDH_SPB(curr_flpfcb->qdh));
				changed=true;
...
static FileNum Sub_FileNumber(uw8 *name, FileNum DirFn, int DirLen, int isdir, int *nlen)
{
	struct fileHeader *h;
	uw32 *z;
	uw16 i,j,n,l;
...
if (j>0 && *(name+2+j)=='_' && j==RW(R_FNM(h)) && GET_FTYP(h)==255)
{
		  FileNum subDirFn;
...
int QDiskOpenDir(struct mdvFile *f)
{
	...
	SET_FNUM(f,fn);
#else
	SET_FNUM(f, FileNumber(NAME_REF(f),1,&nlen));
#endif
	WW(NAME_REF(f),nlen);
...
static OSErr QLWA_CreateNewFile(struct mdvFile *f)
{
	...
	fn0=root_fn();
#else
	fn0=FileNumber(NAME_REF(f),1,&dummy);
#endif
	
	if(QWA_FC(curr_flpfcb->qdh)<1) return QERR_DF;
...
slotFound:
	...
	z=(uw32*)p;
	for(j=0;j<16;j++) WL(z++,0);    /* write header copy ???? */
	PutSector(p);
	
	p=GetFileSector(fn0,i>>3);        /* get sector containing direntry */
	if(p==nil) goto errore;
	
	h=(struct fileHeader *)(p+((i&7)<<6));   /* setup direcory entry */
	
	for(i=0;i<16;i++) WL((char*)h+i*4,0);
	SET_FLEN(h,64);
	l=(uw16)RW((uw16*)NAME_REF(f))+2;
	if(l>36) l=36;
	BlockMoveData(NAME_REF(f),R_FNM(h),(uw16)RW((uw16*)NAME_REF(f))+2);
	time=ReadQlClock();
	SET_FDTBAC(h,time);
	SET_FDUPDT(h,time);
...
static OSErr CreateNewFile(struct mdvFile *f)
{
	...
	if(QDH_FREE(curr_flpfcb->qdh)<1) return QERR_DF;
			...
			{	
				...
				{	
					...
					{	
						...
						i++;
					} while(i<l && c!=10 && GET_POS(f)<GET_EOF(f));
					l=i;
					if(c==10) e=LF_FOUND;
				...
				{	
					...
					BlockMoveData(s+(GET_POS(f)&511),p,l);
					p+=l;
					SET_POS(f,GET_POS(f)+l);
				}
				nn-=l;
	...
	switch(op){
		...
		case 5:			/* send byte */
			...
			case 0x45:		/* medium information */
				if (is_qlwa())
				{
					reg[1]=((w32)QWA_FC(curr_flpfcb->qdh)<<16)*sect_per_cluster()+QWA_CC(curr_flpfcb->qdh)*sect_per_cluster();
					p=(char*)(curr_flpfcb->qdh)+10;
					for(i=0;i<10;i++) WriteByte(aReg[1]++,*p++);
				}
				else
				{
					reg[1]=((w32)QDH_FREE(curr_flpfcb->qdh)<<16)+QDH_TOTAL(curr_flpfcb->qdh);
					p=(char*)(curr_flpfcb->qdh)+4;
					for(i=0;i<10;i++) WriteByte(aReg[1]++,*p++);
				}
				break;
				...
				{	
					...
					*reg=QERR_BM;	/* bad medium */
					...
					{	uw32 a1=aReg[1];
						w=(w16*)((Ptr)h+4);
						aReg[1]+=4;
						for(i=0;i<5;i++,aReg[1]+=2) WW(w++,ReadWord(aReg[1]));
...
#if 1
					{
						...
						if (GET_FILESYS(f)>=0)
						{
							strncpy((char*)theROM+aReg[1]+2,
									(p = qdevs[GET_FILESYS(f)].mountPoints[GET_DRIVE(f)]),20);
						}
						else
						{
							strcpy((char*)theROM+aReg[1]+2,(p = "uQVFSx root"));
						}
						nl = strlen(p);
						...
						free = is_qlwa() ? QWA_FC(curr_flpfcb->qdh): QDH_FREE(curr_flpfcb->qdh);
						mxs =  is_qlwa() ? QWA_CC(curr_flpfcb->qdh): QDH_TOTAL(curr_flpfcb->qdh);
...
OSErr QDiskDelete(struct mdvFile *f) /* struct mdvFile passed, but only the name is valid (not the fNumber) and the drive */
{
	...
	if(curr_flpfcb->buffer==nil || !curr_flpfcb->isValid) return ERR_UNINITIALIZED_DISK;
	SET_FNUM(f,fileNum=FileNumber(NAME_REF(f),0,&dummy));
	if(fileNum.file>0)
    {	
		err=0;
		SET_OPEN(f,false);
...
{	
	...
	if(error)
		BlockMoveData(ql5a? format_table_dd:format_table_hd,ltp,36);
}
