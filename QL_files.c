...
void FilenameFromQL(unsigned char *fName)
{
  ...
  *fName=0;
  l=fName[1];
  if(l>31) l=fName[1]=31;
  if(l==0) BlockMoveData("-noname-",fName+1,9);
  else
    {	/* examine filename for non ascii characters */
      for(i=1;i<=l;i++)
	{
	  c=fName[1+i];
	  if(c<32 || c==':')	/* || c>127 */
...
static void NameToQL(Str255 name)
{
  ...
  while(l>0)
	...
	{
	  name[++j]=(*p++)-'0';
	  if(name[j]>9) name[j]-=7;
	  l--;
	  if(l>0 && *p!=32)
	    {
	      name[j]=(name[j]<<4)+(*p>'9'? *p+10-'A':*p-'0');
	      l--;
	    }
	  p++;
	  l--;
...
static Cond HasName(Str255 name)
{
  ...
  if(*p++!=8) return true;
  if(*p++!='-') return true;
  if(((*p++)&223)!='N') return true;
  if(((*p++)&223)!='O') return true;
  if(((*p++)&223)!='N') return true;
  if(((*p++)&223)!='A') return true;
  if(((*p++)&223)!='M') return true;
  if(((*p++)&223)!='E') return true;
  if(*p!='-') return true;
  return false;
}
...
static void InitDirDevDriver(Str255 name,w32 addr,w32 *fsid)
{
  ...
  reg[2]=0;
  QLtrap(1,0x18,20000l);		/* allocate memory for the driver linkage block */
  if((*reg)==0)
    {
#if 1
      *fsid=aReg[0];
      ...
      *p=0;
      ...
      QLtrap(1,0x22,20000l);	/* link directory device driver in IOSS */
    }
...
void InitFileDrivers ()
{
	...
	if (qdevs[i].qname)
	{
	  ...
	  for (j=0; j<8; j++) if (qdevs[i].Present[j]) break;
	  if (j<8)
	    InitDirDevDriver (qdevs[i].qname, 0x14000, (w32 *)&qdevs[i].ref);
	}
    }
...
int file_reopen(struct mdvFile * f, int frk)
{
  ...
  if (frk) next= GET_NEXT(f);
  
  res=mdv_doopen(f, filesys, drive, key, pdb);

  if (frk) SET_NEXT(f,next);
...
void MdvIO(void)
{
  ...
  if ((GET_ID(f)!=MDV_ID) || (!GET_OPEN(f)))
    {
      ...
      if (file_reopen(f,0)<0) goto xerr;
  ...
  *reg=0;
  ...
  if(GET_ISDIR(f) && (op==5 || op==7 || op==0x49 || op==0x46)) debug2("Writing to directory. op=",op);
  isdisk = GET_ISDISK(f);
  if( isdisk == 0 || isdisk == 2) (void)QHostIO(f,op,isdisk);
    else QDiskIO(f,op);
...
uw32 mdv_doopen(struct mdvFile * f, int filesys, int drive, int key, uw32 pdb)
{
  ...
  
  {
    ...
    if(qdevs[filesys].Present[drive])
      {   
#ifdef TRACE_OPEN
	printf("MDVOpen : name %s namelen %d %s\n",NAME_REF(f)+2,RW(NAME_REF(f)), (key==4 ? "as directory":""));
#endif
	...
	SET_KEY(f,key);
	res=0;
	if(key!=Q_DIR && !onDisk) FilenameFromQL(NAME_REF(f));
	if(key>=Q_DIR || key<0)
	  {
		...
		else  res=QDiskOpenDir(f);
		/* printf("OpenDir retval %d\n",res); */
	      }
	    else if(key<0)	/* delete file */
	      {
		if(onDisk == 0 || onDisk == 2)
		  res=HDelete(mdvVol[drive],mdvDir[drive],NAME_REF(f),f,2);
		else 
		  res=QDiskDelete(f);
	      }
	    else res=-15; /* bad parameter */
	  }
	else				/* open file */
	  {	
	    ...
	    if( onDisk == 0 || onDisk == 2 )
	      {
		perm=( key==1 ? O_RDONLY : O_RDWR ) | (canExist && canCreate ? O_CREAT : 0 );
		e=HOpenDF(mdvVol[drive],mdvDir[drive],NAME_REF(f),perm,f,0,onDisk);

		if (e && (key !=2)) {
		  perm= O_RDONLY | (canExist && canCreate ? O_CREAT : 0 );
		  e=HOpenDF(mdvVol[drive],mdvDir[drive],NAME_REF(f),perm,f,0,onDisk);
		}
		...
		if(e==0 && !canExist)
		  ...
		  if(e==0) /* file succesfully opened */
		      SET_ISDIR(f,false);
		  else
		      {
			if(e==-49) res=-9; /* in use */
			else
			  {	
			    res=QERR_NF;
			    if(canCreate)
				{
				  /*printf("file name %s\n",NAME_REF(f)+2);*/
				  perm=(key==1 ? O_RDONLY : O_RDWR ) | (canExist && canCreate ? O_CREAT : 0 );
				  e=HOpenDF(mdvVol[drive],mdvDir[drive],NAME_REF(f),perm|O_CREAT,f,canCreate,onDisk);
				  if (e && (key !=2)) {
				    perm=O_RDONLY | (canExist && canCreate ? O_CREAT : 0 );
				    e=HOpenDF(mdvVol[drive],mdvDir[drive],NAME_REF(f),perm,f,0,onDisk); 
				  }
 ...
 noBlock:
  if(res==0 && GET_KEY(f)>=0)
	...
	if (qdevs[filesys].Where[drive]==1 )
	...
	else ftruncate(GET_HFILE(f),0); 
	
      SET_POS(f,64); /*f->pos=64;*/
      if( onDisk == 0 || onDisk == 2) SET_EOF(f,HDfLen(f,onDisk)+64);
      else SET_EOF(f,QDiskLen(f)+64);
      ...
      if ( onDisk==0 || onDisk==2 ){ /* linking for QDisk not necessary */
...
void fork_files()
{
  ...
  for(fs=0;fs<MAXDEV;fs++)
	...
	{
	  if ( (qdevs[fs].Where[dr]==0 || qdevs[fs].Where[dr]==2 )
...
Cond FileAlreadyOpen(short drive,int filesys,FileNum fileNum,w16 *perm) /* works for ql disks only */
{
  struct mdvFile *f;
...
static void UnlinkOpenFile(short drive,struct mdvFile *f)
{
  ...
  if(p==f) qdevs[filesys].FileList[drive]=GET_NEXT(f);
  else
    {
      while(p!=nil && GET_NEXT(p)!=f) p=GET_NEXT(p);
      if(GET_NEXT(p)==f) SET_NEXT(p,GET_NEXT(f));
    }
  isdisk = GET_ISDISK(f);
  if (isdisk == 0 || isdisk == 2){
    fcb=qdevs[filesys].fcbList[drive];
    if (fcb==fb) qdevs[filesys].fcbList[drive]=fcb->next;
    else {
      while(fcb && fcb->next !=fb) fcb=fcb->next;
      if (fb == fcb->next) fcb->next=fb->next;
    }
  }
}
...
void MdvClose(void)
{
  ...
  if(GET_OPEN(f))
    {
      if(GET_ISDISK(f) == 1) QDiskClose(f);
      else FSClose(GET_HFILE(f));
    }
...
void MdvFormat(void)
{
  ...
  *reg=QERR_NI;
#else
  if(mdvOn)	*reg=-9;	/* in use */
  else
    {
      drive=(w8)reg[1];	/* drive number */
      if(drive<1 || drive>2) *reg=-7;	/* not found */
      else 
	{	
	  filesys=1;drive--; 
	if(qdevs[filesys].Present[drive] && qdevs[filesys].OpenFiles[drive]==0)
	  {
	    w32 a6;

	    if(!qdevs[filesys].Where[drive]) NoteAlert(rFormat,nil);
	    /* set medium name */
	    nameLen=ReadWord(aReg[1])-5;
	    if(nameLen>255) nameLen=255;
	    else if(nameLen<0) nameLen=0;
	    flpName[drive][0]=nameLen;
	    if(nameLen>0) BlockMoveData((Ptr)theROM+aReg[1]+7,&(flpName[drive][1]),nameLen);
	    if(nameLen>10) nameLen=10;
	    if(nameLen>0) BlockMoveData((Ptr)theROM+aReg[1]+7,mdvHeaders+14*drive+2,nameLen);
	    while(nameLen<10) mdvHeaders[14*drive+2+nameLen++]=' ';
	    
	    /* set header flag, sector and random number */
	    mdvHeaders[drive*14]=0xff;
	    mdvHeaders[drive*14+1]=0;
	    a6=0x28000;
	    mdvHeaders[drive*14+12]=*((uw8*)theROM+a6+0x2e);
	    mdvHeaders[drive*14+13]=*((uw8*)theROM+a6+0x2f);
	    /* set number of total/good sectors */
	    *reg=0;
	    if(qdevs[filesys].Where[drive]) QDiskFormat(mdvHeaders+14*drive+2,&total,&empty);
	    else
	      *reg= QERR_NI;
	    ...
	    if(*reg==0)
	      {
		reg[1]=empty;
		if(reg[1]>30000) reg[1]=30000;
		reg[2]=total;
		if(reg[2]>30000) reg[2]=30000;
	      } else qdevs[filesys].Present[drive]=false;
	    /*      InvalWindow(mdvWindow); */
	  }
	else
	  {
	    if(qdevs[filesys].OpenFiles[drive]!=0) *reg=-9; /* in use */
	    else *reg=-14;	/* format failed */
	  }
	drive++;
	}
    }
...
{
  switch(d){
  case 3:mdvOn=1;DrawMdvLeds();break;
  case 2:
    if(mdvOn!=0)
      {
	mdvOn++;
...
{
  if((uw16*)((Ptr)theROM+0x4002+RW((uw16*)((Ptr)theROM+0x124)))==gPC)
    {
      if(mdvOn==1 || mdvOn==2)
	{
	  AddReturn(2);
...
{
  if((uw16*)((Ptr)theROM+0x4002+RW((uw16*)((Ptr)theROM+0x126)))==gPC)
    {
      if(mdvOn==1 || mdvOn==2) aReg[1]+=512;
      rts();
    }
  else
    {
...
{
  if((uw16*)((Ptr)theROM+0x4002+RW((uw16*)((Ptr)theROM+0x128)))==gPC)
    {	
      if(mdvOn==1 || mdvOn==2)
	{
	  AddReturn(2);
...
void ReadMdvHeader(void)
{
  short	i;
  w8*		p;
  if((uw16*)((Ptr)theROM+0x4002+RW((uw16*)((Ptr)theROM+0x12a)))==gPC)
    {
      if(mdvOn==1 || mdvOn==2)
	{
	  AddReturn(4);
	  p=(w8*)mdvHeaders;
	  if(mdvOn==2) p+=14;
	  for(i=0;i<14;i++) WriteByte(aReg[1]++,*p++);
