...
void LoadCPUInstructions(void)
{
	FILE	*fp;

	char	buf[PATH_MAX+10], Mask[17], Name[15], Mnem[8];
	char 	*p;
	...
	RecNo=1; NmLen=FnLen=0;
	while(!feof(fp))
	{
		ScanRet=fscanf(fp,"%*2s,%16s,%d,%[^,],%[^,],%x,%x\n",Mask,&Idx,Name,Mnem,&ArgL,&ArgR);
		if (ScanRet!=6) { printf("Format error at record %d field %d. Stopping.\n",RecNo,ScanRet+1); exit(1); }

		printf("%d: %s %03u %16-s %8-s %04x %04x\n", RecNo++, Mask, Idx, Name, Mnem, ArgL, ArgR );
	}
}
