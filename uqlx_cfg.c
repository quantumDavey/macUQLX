...
static PARSELIST pl[32];

static void make_parselist()
{
	...
	PARSELIST pl_source[] = {
		...
		{"ROMIM",  (PVFV) ParseList, offsetof(QMDATA, romlist), ParseROM},
		...
		{"RAMTOP", (PVFV) pInt4, offsetof(QMDATA, ramtop), NULL },
		{"COLOUR", (PVFV) pInt2, offsetof(QMDATA, color), NULL },
		{"CPU_HOG",(PVFV) pInt2, offsetof(QMDATA, cpu_hog), NULL },
		{"REAL_WHITE",(PVFV) pInt2, offsetof(QMDATA, fwhite), NULL },
		{"FAST_START",(PVFV) pInt2, offsetof(QMDATA, fastStartup), NULL },
		{"XKEY_ON",(PVFV) pInt2, offsetof(QMDATA, xkey_on), NULL },
		{"STRICT_LOCK",(PVFV) pInt2, offsetof(QMDATA, strict_lock), NULL },
		{"NO_PATCH",(PVFV) pInt2, offsetof(QMDATA, no_patch), NULL },
		{"DO_GRAB",(PVFV) pInt2, offsetof(QMDATA, do_grab), NULL },

		{"XVID",(PVFV) pInt2, offsetof(QMDATA, pref_vid), NULL },
		{"XDEPTH",(PVFV) pInt2, offsetof(QMDATA, pref_depth), NULL },
	...
	memset(pl, NULL, sizeof(pl));

	while (pl_source[pl_idx].id != NULL ) {
		strcpy(pl[pl_idx].id, pl_source[pl_idx].id);
		pl[pl_idx].func = pl_source[pl_idx].func;
		pl[pl_idx].offset = pl_source[pl_idx].offset;
		pl[pl_idx].mx = pl_source[pl_idx].mx;
		pl_idx++;
	}
}
