...
{   
	register bctype c;
		...
		if(((char)addr&1)!=0)
		{   
			exception=3; extraFlag=true; nInst2=nInst; nInst=0; badAddress=addr; readOrWrite=16;
			return 0;
		}
		addr&=ADDR_MASK_E;
	#else
#warning HOST_ALIGN <=1
		addr &=ADDR_MASK;
	#endif
...
{   
  ...
  if(c!=2)
	...
	{ 
	  ...
	  switch(addr)
	  {
		  case 0x018063:	SetDisplay(d,true);									// Display control
		  case 0x018000:
		  case 0x018001:														// ignore write to RTC regs
		  case 0x018023:	break;												// ignore write to no reg
							
		  case 0x018002:	if(d!=16)
							{	debug2("Write to transmit control >",d);
								debug2("at pc-2 ",(Ptr)pc-(Ptr)theROM-2); }		//TRR
							break;
			  
		  case 0x018003:	debugIPC("Write to IPC link > ",d);
							debugIPC("at (PC-2) ",(Ptr)pc-(Ptr)theROM-2);
							break;												//TRR
			  
		  case 0x018020:	WriteMdvControl(d);  break;							//TRR
			  
		  case 0x018021:	WriteInt(d); break;
			  
		  case 0x018022:	debug2("Write to MDV/RS232 data >",d); break;		//TRR
			  
		  default:			debug2("Write to HW register ",addr);				//TRR
