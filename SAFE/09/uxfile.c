...
void FillXH(int fd,char *name,struct fileHeader *h, int fstype)
{
  ...
  cmp_fun = fstype ? strncasecmp : strncmp;
  while (64==read(fd,&buf,64))
    if (nlen==RW((Ptr)REF_FNAME(&buf)) && !cmp_fun(name,REF_FNAME(&buf)+2,nlen) && RL((Ptr)&buf)){
...
void setheader(char *fsmount, char *uxname, struct fileHeader *h,int fstype)
{
  ...
  while (64==read(ff,&buf,64)) {
    if (!cmp_fun(dpath,REF_FNAME(&buf)+2,RW((Ptr)REF_FNAME(&buf)))
...
definition in unix.h",template);
  ...
  while ((dp=readdir(dirp))!=NULL)
    {
      ...
      strncpy(REF_FNAME(&h)+2+qplen,dp->d_name,37-min(qplen,32));
  ...
  lseek(fd,0,SEEK_SET);
  SET_FLEN(&h,i); WL(((Ptr)&h)+4,0);WL(((Ptr)&h)+8,0);WW((Ptr)REF_FNAME(&h),0);
  SET_FTYP(&h,255);
  write(fd,&h,64);
#endif
  ...
  switch(op)
	...
	*reg=-10;
      ...
      if (count<4)
	{
      ...
      QHFillHeader(&hh,FILDES,(pvf)fd,NULL,f,fstype);
#if 1
      strncpy(REF_FNAME(&hh)+2,(Ptr)NAME_REF(f)+2,min(36,RW(NAME_REF(f))));
