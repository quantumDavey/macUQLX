...
#ifndef IE_XL
#warning  IE_XL not defined here
...
#warning IE_XL IS undefined
#else
#warning IE_XL IS defined
#endif  /* IE_XL */

#ifdef IE_XL
#define LR  &&
#else
#warning UNUSED IE_XL
#define LR
#endif
...
void SetTable(void (**otbl)(void), syntab *stbl, char *mask,void (*f)(void), char *mnem, int smask, int dmask)
{
	...
	/*if(V3) printf("SetTable: insns %s, *code %x\n",mask,f);*/
	   	...
	   	while(c2--)
		{
			...
			while(i--)
			{
				*p=f;
				...
				/*if (V4) printf("setting %06d",p-otbl);*/
				p+=m0;q+=m0;
			}
   	...
   	else
   	{
		...
		while(c1--)
		{
			*p=f;
			strncpy(q->m, mnem, 7);
			q->s=smask; q->d=dmask; q->x=0;
			if (V4) printf("%05d %s 0x%04x 0x%04x \n",q-stbl, q->s, q->d );
			p+=m0;
		}
   	}
}
...
void SetInvalEntries(void (**instbl)(void), syntab syntbl[], void *code, char *mnemonic, int arg1mask, int arg2mask)
{
	...
	{
		instbl[i]= code;
		strncpy(syntbl[i].m, mnemonic, 7);syntbl[i].s=arg1mask;syntbl[i].d=arg2mask;syntbl[i].x=0;
