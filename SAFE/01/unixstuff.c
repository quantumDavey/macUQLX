...
void uqlx_prestore(unsigned long start, unsigned long len)
{
  ...
  /*printf("uqlx_prestore %x,%x\n",start,len);*/

  for (i = start>>pageshift; (start+len) > (i<<pageshift); i++) 
    {
      switch(RamMap[i])
	{
	case QX_NONE: prot=PROT_READ; break;
	case QX_ROM:  prot=PROT_READ; break;
	  /*case QX_QXM:  prot=PROT_READ; break;*/
	case QX_RAM:  prot=PROT_READ|PROT_WRITE; break;
	case QX_SCR:  
	  ...
	  /* printf(".. QX_SCR: %x\n",prot);*/
	  break;
	case QX_IO:   prot=is_patching ? PROT_NONE : PROT_READ; break;
	default: printf("illegal value in uqlx_prestore\n");
	}
#ifndef EVM_SCR
...
void uqlx_protect(unsigned long  start, unsigned long len, int type)
{
...
#if 0 //defined(USE_VM) || defined(VM_SCR)
  // just a hack to speedup boot -- results in lost updates??
  if (type == QX_SCR) tmp=QX_RAM;
  else tmp=type;
#else
  tmp=type;
#endif
...
#if defined(USE_VM) || defined(VM_SCR)
  {
	...
	/*case QX_QXM:  prot=PROT_READ; break;*/
      case QX_RAM:  prot=PROT_READ|PROT_WRITE; break;
      case QX_SCR:  prot=PROT_READ; break;
      case QX_IO:   prot=is_patching ? PROT_NONE : PROT_READ; break;
      default: printf("illegal value in uqlx_protect\n");
	  }
    if (MPROTECT((Ptr)theROM+start,len,prot))
      perror("could not change VM permission");
  }
...
#else /* EMX */
  timerthread=_beginthread(dotime,NULL,32767,NULL);
  DosSetPriority(PRTYS_THREAD,PRTYC_FOREGROUNDSERVER,PRTYD_MAXIMUM,timerthread);
#endif
#endif
}
...
#if defined(VM_SCR) || defined(USE_VM)
#ifdef SPARC
  sac.sa_sigaction=segv_handler;  /* SUN seem to be the only to do this..*/
#else
...
printf("PC=%x\tSP=%x\tSSP=%x\n",(void*)pc-(void*)theROM,*sp,ssp);
  /*DbgInfo(); */
